...
# Load MCMC data
import os 
basedir="./mcmcdata/tauconstrain"
allfiles=os.listdir(basedir)
allfiles.sort()
parsfiles=[i for i in allfiles if "params" in i]
chifiles=[i for i in allfiles if "chisq" in i]
parsdata=[np.load(f"{basedir}/{i}").T for i in parsfiles]
chidata=[np.load(f"{basedir}/{i}").T for i in chifiles]
...
for par,meanpar,varpar,name in zip(pars,mean_pars,var_pars,parnames):
    print(f"\npar={par}\nmeanpar={meanpar}")
    plt.plot((par-meanpar)/sqrt(varpar),".",markersize=3,alpha=1.0,label=name)
plt.legend()
plt.title("Sigma normalized parameters")
plt.xlabel("MCMC time")
plt.ylabel("Parameter value")
plt.savefig(f"mcmcplot/all_params_p4.png",dpi=450)
...
plt.savefig(f"mcmcplot/all_params_fft_p4.png",dpi=450)
plt.show(block=True)
